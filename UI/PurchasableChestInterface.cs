...
using RiskOfTerrain.Content.Chests;
...
public class PurchasableChestInterface : ModSystem {
    public class ChestPurchasePopup {
        ...
        public Rectangle TextHitbox => new Rectangle((int)position.X - textWidth / 2, (int)position.Y - textHeight / 2, textWidth, textHeight);
        ...
        public bool Update() {
            if ((!Chest.IsLocked(chestX, chestY) && TileLoader.GetTile(Main.tile[chestX, chestY].TileType) is not UnifiedSecurityChest.InstancedLockedChest) || Vector2.Distance(Main.LocalPlayer.Center, new Vector2(chestX * 16f + chestCenterOffset.X, chestY * 16f + chestCenterOffset.Y)) > radius) {
    ...
    public override void ModifyInterfaceLayers(List<GameInterfaceLayer> layers) {
        if (PurchasePopups.Count > 0) {
            int index = layers.FindIndex((l) => {
                return l.Name == "Vanilla: Entity Health Bars";
            });
            if (index != -1)
