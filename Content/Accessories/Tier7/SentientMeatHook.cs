    ...
    public override void OnHit(EntityInfo entity, EntityInfo victim, Entity projOrItem, NPC.HitInfo hit) {
        if (victim.entity is NPC npc && entity.RollLuck(5) == 0) {
            for (int i = 0; i < Main.maxNPCs; i++) {
                if (RORNPC.Distance(Main.npc[i], npc) <= 400 && !Main.npc[i].boss && !Main.npc[i].ROR().isBossMinion && !Main.npc[i].friendly && Main.npc[i].lifeMax > 5 && Main.npc[i].damage > 0 && i != npc.whoAmI && Main.npc[i].aiStyle != NPCAIStyleID.Worm) {
